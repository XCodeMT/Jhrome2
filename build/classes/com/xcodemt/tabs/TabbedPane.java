...
import com.xcodemt.tabs.jhrome.JhromeContentPanelBorder;
import com.xcodemt.tabs.jhrome.JhromeNewTabButtonUI;
...
/**
 * {@link TabbedPane} is a Google Chrome-like tabbed pane, providing animated tab layout, drag and drop capabilities, and a new tab button. All Google Chrome behavior is provided by default. Tabs can
 * be dragged around within {@code TabbedPane} and rearranged, they can be "torn away" or dragged out and opened in new windows, and they can be dragged from one window to another. If all the tabs in
 * a {@code TabbedPane} are closed or torn away, the window containing that {@code TabbedPane} is disposed. When a tab is torn away, a ghosted drag image window showing the tab and its contents will
 * appear and follow the mouse cursor until the tab is dragged over another {@code TabbedPane} or dropped.<br /> <br />
 *
 * Animation includes tabs expanding when added, contracting when removed, jumping around when being reordered, contracting simultaneously when there is not enough room, and expanding simultaneously
 * when there is room again and the mouse is no longer on top of the tab zone.<br /> <br />
 *
 * {@code TabbedPane} is designed to allow you to customize the look and behavior as much as possible. The following interfaces help with customization: <ul> <li>{@link ITab} provides an interface to
 * tab renderers/content. You can use literally any {@link Component} (or combination of {@code Component}s) in a tab renderer by providing them through an {@code ITab} implementation.
 * <li>{@link ITabFactory} creates new tabs when the new tab button is clicked. By providing your own tab factory you can use any kind of tabs you like with {@code TabbedPane}.</li>
 * <li>{@link ITabbedPaneDnDPolicy} controls whether tabs may be torn away or snapped back in (by default both are always allowed). By providing your own DnD policy you can create arbitrarily complex
 * behavior, preventing only certain tabs from being torn away, certain tabs from being snapped in, only at certain times, etc.</li> <li>{@link IFloatingTabHandler} determines what happens when a tab
 * is torn away from this {@code TabbedPane} and is "floating." {@link XFloatingTabHandler} shows a ghost drag image of the tab in another window, but you may create different behavior by providing
 * your own {@link IFloatingTabHandler}. <li>{@link ITabDropFailureHandler} determines what to do when a tab drop fails (i.e. it is dropped on the desktop or another application that rejects the
 * drop). The {@link XTabDropFailureHandler} creates a new window for the tab, but you can do something else by providing your own {@link ITabDropFailureHandler}. </ul>
 *
 * Also, {@code TabbedPane} allows you to get its new tab button and content pane so that you can modify them arbitrarily.<br /> <br />
 *
 * Since the layout is animated, when you remove a tab from {@code TabbedPane} (using {@link #removeTab(ITab)}), it makes it start contracting and doesn't actually remove the tab renderer component
 * until it is done contracting. However, all public methods that involve the tab list behave as if tabs are removed immediately. For example, if you remove a tab, it will no longer show up in
 * {@link #getTabs()}, even while the tab renderer component is still a child of this {@code TabbedPane} and contracting. If you add the tab back before it is done contracting, it will jump to the new
 * position and expand back to full size.
 *
 * @author andy.edwards
 */
...
public class TabbedPane extends JLayeredPane {
    ...
    private static class TabInfo {
        ...
        int vTargetX;
        /**
         * The tab's target width in virtual coordinate space. This does not include the overlap area -- it is the distance to the virtual target x position of the next tab. To get the actual target
         * width, this value will be scaled down and the overlap amount will be added.
         */
        int vTargetWidth;
        /**
        ...
        int vCurrentX;
        /**
         * The tab's current width in virtual coordinate space. This does not include the overlap area -- it is the distance to the virtual current x position of the next tab. To get the actual
         * current width, this value will be scaled down and the overlap amount will be added.
         */
        int vCurrentWidth;
        /**
        ...
        int dragX;
        /**
         * The relative x position at which the tab was grabbed, as a proportion of its width (0.0 = left side, 0.5 = middle, 1.0 = right side). This way if the tab width changes while it's being
         * dragged, the layout manager can still give it a reasonable position relative to the mouse cursor.
         */
        double grabX;
    }
    ...
    /**
     * How many pixels the content panel overlaps the tabs. This is necessary with the Google Chrome appearance to make the selected tab and the content panel look like a contiguous object
     */
    ...
    private ITabDropFailureHandler tabDropFailureHandler = new XTabDropFailureHandler();
    private IFloatingTabHandler floatingTabHandler = new XFloatingTabHandler();
    private ITabFactory tabFactory = new XTabFactory();
    ...
    /**
     * Sets the drop failure handler to a {@link XTabDropFailureHandler} with the given window factory.
     */
    public void setWindowFactory(ITabbedPaneWindowFactory windowFactory) {
        setDropFailureHandler(new XTabDropFailureHandler(windowFactory));
    }
    ...
    public void setDropFailureHandler(XTabDropFailureHandler dropFailureHandler) {
    ...
    /**
     * @return the width to make each tab if {@link #isUseUniformWidth()} is {@code true}. If there are too many tabs in this tabbed pane to fit at this width, they will of course be squashed down.
     */
    ...
    /**
     * Sets the width to make each tab if {@link #isUseUniformWidth()} is {@code true}. If there are too many tabs in this tabbed pane to fit at this width, they will of course be squashed down.
     */
     ...
     *
     * @throws IllegalArgumentException if {@code tab} is already a member of this tabbed pane.
     */
     ...
     *
     * @param index the index to add the tab at. Like {@link List#add(int, Object)}, this method will insert the new tab before the tab at {@code index}.
     * @param tab the tab to add.
     *
     ...
     * @param newIndex the index to move the tab to. Like {@link List#add(int, Object)}, this method will insert {@code tab} before the tab at {@code newIndex}.
     *
    ...
    /**
     * Removes a tab from this tabbed pane with animation. The layout will animate the tab renderer shrinking before it is actually removed from the component hierarchy. However, the tab will be
     * immediately removed from the list of tabs in this tabbed pane, so {@link #getTabs()}, {@link #addTab(ITab)},
     * {@link #moveTab(ITab, int)}, etc. will all behave as if it is no longer a member of this tabbed pane. If you want to remove the tab and its renderer component immediately, use
     * {@link #removeTabImmediately(ITab)}.
     *
     * If the selected tab is removed, one of the adjacent tabs will be selected before it is removed. If it is the only tab, the selection will be cleared before it is removed.
     *
    ...
    /**
     * Removes a tab from this tabbed pane without animation. This means the tab renderer will be removed from the component hierarchy immediately, unlike {@link #removeTab(ITab)}.
     *
     * If the selected tab is removed, one of the adjacent tabs will be selected before it is removed. If it is the only tab, the selection will be cleared before it is removed.
     *
    ...
    /**
     * Removes all tabs from this tabbed pane without animation. This method is equivalent to setting the selected tab to {@code null} and then removing all tabs one by one.
     */
     ...
     *
     * @param tab the new selected tab. If {@code null} is given, the selection will be cleared.
     *
     * @throws IllegalArgumentException if {@code tab} is non-{@code null} but not a member of this tabbed pane.
     */
    ...
    /**
     * Notifies this {@code TabbedPane} that a tab's content has changed. If that tab is selected, its old content will be removed from this {@code TabbedPane} and replaced with the tab's new content.
     *
     * @param tab the tab whose content has changed. If {@code tab} is not a member of this tabbed pane, this method has no effect.
     */
    ...
    /**
     * @return the new tab button. This tabbed pane manages its appearance and action by default. You are free to modify it however you wish, but behavior is undefined in this case so be careful.
     */
    ...
    /**
     * @return the content panel. This tabbed pane adds the tab content to it automatically. You are free to modify it however you wish, but behavior is undefined in this case so be careful.
     */
    ...
    /**
     * Removes all tabs, stops animation and attempts to unregister all listeners that could prevent this TabbedPane from being finalized. However, I am not yet certain if this method eliminates all
     * potential memory leaks (excluding references apart from internal TabbedPane code.
     */
    public void dispose() {
        checkEDT();
    ...
    private class TabLayoutManager implements LayoutManager {
        ...
        public void layoutContainer(Container parent) {
            ...
            /**
             * The target width of the tab zone, which is the total target width of all tabs, except those being removed, in virtual coordinate space. This does not include the overlap area of the
             * last tab.
             */
            int vTargetTabZoneWidth = 0;

            /**
             * The current width of the tab zone, which is the total current width of all tabs, including those being removed, in virtual coordinate space. This does not include the overlap area of
             * the last tab.
             */
