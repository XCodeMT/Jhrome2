...
/**
 * The interface for a jhromeTab in a {@link TabbedPane}. {@code JhromeTabbedPane} uses this interface to interact with the jhromeTab and its content. This allows you to use any {@link Component} for
 * the jhromeTab, and make it behave as though it has a non-rectangular shape.
 */
public interface ITab {

    /**
     * @return the {@link Component} that represents the jhromeTab itself. This will not merely paint the jhromeTab, like a single {@link ListCellRenderer} is used to paint many cells, but will
     * actually be added to a {@link TabbedPane}'s Component hierarchy.
     */
    public abstract Component getRenderer();

    /**
     * @return the {@link Component} that represents the jhromeTab's content. It will be added to a {@link TabbedPane}'s content panel whenever this jhromeTab is selected. May be {@code null}.
     */
     ...
     * @return {@code true} if the user can drag the jhromeTab after dragging from {@code p}. NOTE: must return {@code false} for points over the close button and other operable components, or the
     * user will be able to drag the jhromeTab from these points!
     */
     ...
     *
     * @param p the point at which the user pressed the mouse, in the renderer's coordinate system.
     * @return {@code true} if the user can select the jhromeTab by pressing the mouse at {@code p}. NOTE: must return {@code false} for points over the close button and other operable components, or
     * the user will be able to select the jhromeTab by clicking these components!
     */
    public abstract boolean isSelectableAt(Point p);

    /**
     * Specifies where the jhromeTab is "hoverable." If the user moves the mouse over the hoverable areas of this jhromeTab, {@link TabbedPane} will set it to the rollover state.
     *
